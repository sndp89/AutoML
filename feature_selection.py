...
def draw_feature_importance(nt_id, mdl_prefix, importance_df):
    ...
    plt.savefig('/home/' + nt_id + '/' + 'mla_' + mdl_prefix + '/' + 'Features selected for modeling.png', bbox_inches='tight')
    plt.close()
    return None

# The module below is used to save the feature importance as a excel file
def save_feature_importance(nt_id, mdl_prefix, importance_df):
    importance_df.drop('idx',axis=1,inplace=True)
    importance_df = importance_df[0:30]    
    importance_df.to_excel('/home/' + nt_id + '/' + 'mla_' + mdl_prefix + '/' + 'feature_importance.xlsx')
    draw_feature_importance(nt_id, mdl_prefix, importance_df)
    return None
