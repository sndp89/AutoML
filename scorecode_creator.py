...
import os
os.chdir('/home/advanl/apps/MLA/auto_model_builder')
...
mdl_prefix = '${mdl_prefix}'

#Since the hdfs and home path below are pointing to your nt_id by default, to use this file for scoring, you need to upload the model objects in hdfs_path and home_path to the appropriate score location path (Could be advanl or any other folder path). You would need the following files to perform scoring.

#hdfs_path  - all the files in the path specified below
#home_path - 'model_scoring_info.z'

hdfs_path = '/user/${nt_id}' + '/' + 'mla_${mdl_prefix}' #update score location hdfs_path 
home_path = '/home/${nt_id}' + '/' + 'mla_${mdl_prefix}' #update score location home_path
""")
...
def selected_model_scorecode(nt_id, mdl_output_id, mdl_prefix, parameters): 
    
    parameters = parameters.substitute(locals())
    scorefile = open('/home/' + nt_id + '/' + 'mla_' + mdl_prefix + '/score_code_selected_model.py', 'w')
...
def individual_model_scorecode(nt_id, mdl_output_id, mdl_prefix, parameters): 
...
model = ${load_model}.load(hdfs_path + '/' + ${i} + '_model.h5')
            ...
            scorefile = open('/home/' + nt_id + '/' + 'mla_' + mdl_prefix + '/' + str(i[0].upper()) + str(i[1:]) + '/score_code_' + i + '_model.py', 'w')
            scorefile.write(import_packages)
            scorefile.write(parameters)
            scorefile.write(import_variables)
